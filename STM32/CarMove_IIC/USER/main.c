...
void Car_Move(void)
{
	...
	switch(state)
	{
		...
		case 2:
		control_speed(300,300,-600,-600);//右旋	Rotate right
		break;
		
		case 3:
		control_speed(-600,-600,300,300);//左旋	Rotate left
		break;
...
{	
	...
	while(1)
	{
		if(times>=250)//定时器每10ms累加一次，当达到250次，即2.5秒的时候转换一次小车的状态	The timer accumulates every 100ms, and when it reaches 25 times, that is, every 2.5 seconds, the state of the car is changed.
		{
            #if MOTOR_TYPE == 4
            Car_Move_PWM();
            #else
